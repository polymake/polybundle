...
You can use polymake in a jupyter notebook. After you have started the bundle once, there is a configuration file \texttt{bundle.config} in the folder \texttt{\$HOME/.polymake-macbundle/} (observe the dot!). In this file you can set the variable \texttt{POLYMAKE_START_IN_JUPYTER} to 1. Note that this requires an installation of \texttt{\bf python3} and \texttt{\bf jupyter} that can be found via your path variable. These two programs are \textbf{not} part of the polymake bundle, you have to install them yourself, e.g. via \texttt{brew} or \texttt{fink}.
...
There are several more variables set in this file to control the run of jupyter
\begin{itemize}
\item \textt{POLYMAKE_JUPYTER_NOTEBOOK_DIR}: This sets the default directory for your jupyter notebook files. By default, this is your home directory. It is highly recommended to set this to some other directory. The given path can either be absolute or relative to your home directory (e.g. \texttt{\tilde/Documents}). Note that the given directory must already exist, it will not be created for you.
\item \texttt{POLYMAKE_JUPYTER_IP}: The ip the server connects to. Usually this is \texttt{localhost}, but you can set this to something else if you start the server remotely. If you start it in a virtual maschine on your computer and want to connect to it from the host, then \texttt{0.0.0.0} is usually the right choice (note that in this case you must forward the jupyter port from the machine to the host).
\item \texttt{POLYMAKE_JUPYTER_PORT}: The port jupyter connects to. Usually \texttt{8888}
\item \texttt{POLYMAKE_JUPYTER_START_BROWSER}: Set this to 0 if jupyter should not start a session in a browser. You should choos this option if you start the server remotely or in a virtual machine. 
\item \texttt{POLYMAKE_JUPYTER_BRWOSER}: Sets the default browser. You need to provide the full path to the binary, followed by \%s. See the configuration file for an example.
\end{itemize}
...
There is one general option that lets you define the terminal app that should be used. By default, polymake starts inside the default \texttt{Terminal.app} that comes with MacOS. If you prefer to use \texttt{iTerm}, then you can set \texttt{POLYMAKE_TERM='iTerm'} in that file. 
