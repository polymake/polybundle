...
### FIXME the current git checkout (April 27, 2014) doesn't compile (GMP flgas missing in some compile command), so we revert to an old one
	@cd $(TMP)/singular/Sources; git checkout `git rev-list -n 1 --before="2014-03-31 00:00" spielwiese`
	@cd $(TMP)/singular/Sources; git archive spielwiese | bzip2 > ../../../src/singular-github-version-branch-spielwiese-2014-03-31.tar.bz2
